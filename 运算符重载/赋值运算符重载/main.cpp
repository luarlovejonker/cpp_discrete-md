...
int main()
{
  ...
  c2.outCom("\t\t\tc2");
  res = c1 + c2;
  res.outCom("run res = c1 + c2 →\tres");
  res = c1.addCom(c2);
  res.outCom("run res = c1.addCom(c2) →\tres");
  res = c1 + 5;
  res.outCom("run res = c1 + 5 →\tres");
  res = 5 + c1;
  res.outCom("run res = 5 + c1 →\tres");
  res = c1;
  c1.outCom("\t\t\tc1");
  res.outCom("run res = c1 →\tres");
  c1.changeReal(-3);
  c1.outCom("run changeReal(-3) →\tc1");
  res.outCom("\t\t\tres");
  res = c1;
